...
```
## let's gamma this one!

```{r}
mod3_g_a <- test_df |> 
  mutate(yem = yem + 1) %$%
  glm(yem ~ 1, family = Gamma(link = "inverse"))
...
tibble(variable = c("employed", "dms", "dag", "ddi", "deh", "dgn"),
  ...
  mutate(model = map2(variable, svy_obj, ~ svyglm(formula(paste("yem ~", .x)), .y)),
         t_val = map_dbl(model, ~ summary(.x)$coefficients[2, "t value"])) |> 
  select(variable, design, t_val) |> 
  ggplot(aes(t_val, variable)) +
...
## Constructing weights as a normal distribution
...
## Constructing weights as a heteroscedastic normal
...
yem_weighted <- test_df |> 
  ...
  mutate(wg0 = dnorm(yem, mean = mean0, sd = sd0),
    ...
    weight_yem = wg0/wg1, .before = year,
    weight_yem = if_else(is.infinite(weight_yem), 1, weight_yem))
