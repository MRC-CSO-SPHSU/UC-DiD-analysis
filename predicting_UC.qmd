---
title: "Prediction models for benefit income"
author: "Andy Baxter"
format: pptx
editor: visual
---
...
print_perc <- function(x) paste0(sprintf("%.1f", 100 * as.numeric(x)), "%")
...
::: columns
::: {.column width="40%"}
-   Multiple pathways of impact of benefits receipt on mental health
-   Ideally a coefficient for each pathway would give an estimate for microsimulation process
-   Not all data are available in APS
    -   Benefit amount received
:::

::: {.column width="60%"}
![](output/mediators_dag.png)
:::
:::
...
# Possibility 2 - Predicting difference
...
# Possibility 3 - Trichotomise

```{r}
model_data_tri <- model_data |> 
  mutate(benefit_change = case_when(
    uc_income == 0 & lba_income == 0 ~ "No change",
...
```{r}
props <- pred_benefit_change %$% table(benefit_change, .pred_class) |> prop.table(margin = 1) 

for (i in 1:3) {
    for (j in 1:3) {
        props[i,j] <- print_perc(props[i,j])
    }
}
