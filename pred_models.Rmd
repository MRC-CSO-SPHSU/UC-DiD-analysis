...
print_perc <- function(x) sprintf("%.1f%%", 100 * x)
...
For this model, $R^2 =$ `r round(uc_rsq, 3)` and root mean squared error $RMSE =$ `r round(uc_rmse, 1)`
...
For this model, $R^2 =$ `r round(lb_rsq, 3)` and root mean squared error $RMSE =$ `r round(lb_rmse, 1)`
...
For this model, $R^2 =$ `r round(bc_rsq, 3)` and root mean squared error $RMSE =$ `r round(bc_rmse, 1)`
...
model_data_tri <- model_data |> 
  ...
  benefit_change = factor(benefit_change)) |> 
...
roc_inc <- pred_benefit_change_prob %$% roc_auc_vec(benefit_change, as.matrix(data.frame(.pred_Decrease, .pred_Increase, `.pred_No change`)))
...
This model has an accuracy of `r print_perc(acc_bc)`, a sensitivity for detecting decreasing benefit changes of `r print_perc(sens_dec)` and a sensitivity of detecting increasing benefits of `r print_perc(sens_inc)` (area under ROC-curve = `r round(roc_inc, 3)`).

### ROC curves

```{r}
pred_benefit_change_prob |> roc_curve(benefit_change, .pred_Decrease, .pred_Increase, `.pred_No change`) |> autoplot()
```
